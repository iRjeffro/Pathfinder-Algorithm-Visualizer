        ...
        prevMaze.map((row, rowIndex) =>
          row.map((cell, cellIndex) => {
            if (rowIndex === y && cellIndex === x) {
              if (cell === "end") {
                window.alert(
                  `Success! End is at cell [${cellIndex + 1}, ${rowIndex +v 1}]`,
                );
              }
  ...
  return (
    <div className={"maze-grid"}>
      <div className={"button-container"}>
        ...
        <button className={"bfs-button"} onClick={() => bfs([1, 0])}>
          Start BFS
        </button>
        <button className={"dfs-button"} onClick={() => dfs([1, 0])}>
          Start DFS
        </button>
      </div>
      <div className={"maze"}>
